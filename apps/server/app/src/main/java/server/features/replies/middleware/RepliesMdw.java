...
public final class RepliesMdw extends BaseMdw {
  ...
  public final Mono<Void> handle(Api api, WebFilterChain chain) {
    return matchPathAfterCutIdOut(api, chain, "/replies", HttpMethod.POST, () -> {
      return limit(api, 10, 15).then(
          withPathId(api).flatMap(
              commentId -> commentSvc.byId(commentId).switchIfEmpty(Mono.error(new ErrAPI("comment not found", 404)))))
          .then(checkBodyForm(api, PostReplyFormT.class).flatMap(typedForm -> userSvc.byId(typedForm.getReplyingTo())
              .switchIfEmpty(Mono.error(new ErrAPI("recipient user not found", 404)))))
